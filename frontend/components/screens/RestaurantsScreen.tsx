...
import { isDefaultTag } from '@/constants/Tags'
...
export default function RestaurantsScreen() {
  ...
  const hasActiveFilters = !!(filterName || filterTags.length || filterFrom || filterTo)
  ...
  const filteredRestaurants = useMemo(() => {
      const filtered = restaurants.filter((r) => {
        ...
        if (filterTags.some((tag) => !(r.tags ?? []).includes(tag))) return false
        return true
      })
      if (activeTab === 'visited') {
        filtered.sort((a, b) => {
    ...
    [restaurants, visitedIds, wishlistIds, activeTab, filterFrom, filterTo, foodStats, visitedCreatedAt, filterName, filterTags],
  )
  ...
  // Every restaurant is already loaded here, so the custom-tag vocabulary comes free.
  const knownTags = useMemo(
    () => Array.from(new Set(restaurants.flatMap((r) => r.tags ?? []))).filter((tag) => !isDefaultTag(tag)),
    [restaurants],
  )
  const canCreateTags = user?.role === 'admin'

  const handleAddToWishlist = async (googlePlaceId: string, tags: string[], comment: string) => {
    const res = await apiClient.post<{ restaurant_id: string; success: boolean }>(
      '/restaurant',
      { google_place_id: googlePlaceId, tags },
    )
  ...
  const handleAddVisitedFromSearch = async (googlePlaceId: string, tags: string[]) => {
    const res = await apiClient.post<{ restaurant_id: string; success: boolean }>(
      '/restaurant',
      { google_place_id: googlePlaceId, tags },
    )
      ...
      >
        <Pressable style={styles.filterOverlay} onPress={() => setFilterModalVisible(false)}>
          <Pressable style={styles.filterModal} onPress={() => {}}>
            ...
            <Text style={styles.filterSectionLabel}>{t.filterByTags}</Text>
